...
UNITS=(
  ...
  "transfer|crates/soulseek-proto/src/transfer.rs|pynicotine/slskmessages.py|file-transfer messages: TransferRequest(40)/TransferResponse(41), QueueUpload(43), PlaceInQueueRequest(51)/PlaceInQueueResponse(44), UploadDenied(50), UploadFailed(46) — exact field layouts, the direction-dependent filesize in TransferRequest (only for UPLOAD=1), and the allowed-vs-reason early-return branches in TransferResponse.parse; plus the F-connection FileMessage framing (FileTransferInit, FileOffset) which are BARE little-endian scalars with no length prefix and no code"
  "distributed|crates/soulseek-proto/src/distributed.rs|pynicotine/slskmessages.py|distributed (D-connection) messages: DistribPing(0), DistribSearch(3), DistribBranchLevel(4, int32), DistribBranchRoot(5), DistribChildDepth(7), DistribEmbeddedMessage(93) — the u8 message code framing (vs u32 for server/peer), the DistribSearch identifier/username/token/searchterm layout, the signed int32 branch level, and the embedded message's inner u8 code + raw body"
)
